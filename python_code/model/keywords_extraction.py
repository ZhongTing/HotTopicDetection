...
def keywords_extraction(articles, method=0, k=10, with_weight=False):
    ...
    if method == 0:
        ...
        return jieba.analyse.extract_tags(input_data, topK=k, withWeight=with_weight, allowPOS=())
    elif method == 2:
        return jieba.analyse.textrank(input_data, topK=k, withWeight=with_weight, allowPOS=('ns', 'n', 'vn', 'v'))
    else:
...
if __name__ == '__main__':
    ...
    print(article.title)
    print(keywords_extraction(article, 0, with_weight=True))
    print(keywords_extraction(article, 1, with_weight=True))
